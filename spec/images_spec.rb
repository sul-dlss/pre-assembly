...
describe Assembly::Images do
  ...
  before(:each) do
    @ai = Assembly::Images.new
  end

  it "should not run if no input file is passed in" do
    @ai.create_jp2('').should be false
  end
  ...
  it "should create jp2 when given a tif" do
    ...
    @ai.create_jp2(TEST_TIF_INPUT_FILE,:output => TEST_JP2_OUTPUT_FILE).should be true
  ...
  it "should not run if the input file is not a tif" do
    ...
    @ai.create_jp2(TEST_JP2_INPUT_FILE).should be false
  end
  ...
  it "should not run if the output file exists and you don't allow overwriting" do
    ...
    @ai.create_jp2(TEST_TIF_INPUT_FILE,:output => TEST_JP2_OUTPUT_FILE).should be false
  end
  ...
  it "should create jp2 of the same filename and in the same location as the input if no output file is specified" do
    ...
    @ai.create_jp2(TEST_TIF_INPUT_FILE).should be true
  ...
  it "should recreate jp2 if the output file exists and if you allow overwriting" do
    ...
    File.exists?(TEST_TIF_INPUT_FILE).should be true
    File.exists?(TEST_JP2_OUTPUT_FILE).should be true
    @ai.create_jp2(TEST_TIF_INPUT_FILE,:output => TEST_JP2_OUTPUT_FILE,:allow_overwrite => true).should be true
    is_jp2?(TEST_JP2_OUTPUT_FILE).should be true
  end
  ...
  it "should not create jp2 if the output profile is not valid" do
    ...
    File.exists?(TEST_TIF_INPUT_FILE).should be true
    File.exists?(TEST_JP2_OUTPUT_FILE).should be false
    @ai.create_jp2(TEST_TIF_INPUT_FILE,:output => TEST_JP2_OUTPUT_FILE,:output_profile => 'junk').should be false
    File.exists?(TEST_JP2_OUTPUT_FILE).should be false
  end
  ...
  it "should not generate valid content metadata if not all input files exist" do
    ...
    File.exists?(TEST_TIF_INPUT_FILE).should be true
    File.exists?(TEST_JP2_INPUT_FILE).should be false
    result = @ai.create_content_metadata(TEST_DRUID,[[TEST_TIF_INPUT_FILE,TEST_JP2_INPUT_FILE]],:content_label => "test label").should be false
  end
