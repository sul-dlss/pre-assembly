...
RSpec.describe PreassemblyJob, type: :job do
  ...
  describe '#perform' do
    it 'requires param' do
      allow(job_run.batch_context.bundle).to receive(:process_digital_objects) # not testing actual work here
    ...
    it 'calls run_pre_assembly and saves job_run.output_location' do
      expect(job_run.batch_context.bundle).to receive(:run_pre_assembly)
      job.perform(job_run)
