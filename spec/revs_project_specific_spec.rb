  ...
  before(:each) do
    ...
    @dobj.desc_md_template_xml = <<-END.gsub(/^ {8}/, '')
      ...
      <mods xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://www.loc.gov/mods/v3" version="3.3" xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-3.xsd">
            ...
            <hierarchicalGeographic>
             ...
             <% end %>
               <% if !manifest_row[:state].blank? %>
                   <state><%=manifest_row[:state]%></state>
                 <% end %>  
        ...
        <% if !manifest_row[:year].blank? %>  
          <originInfo keyDate="yes">
            <dateCreated><%=manifest_row[:year]%></dateCreated>
          </originInfo>
        <% end %>
        <% if !manifest_row[:date].blank? %>
          <originInfo keyDate="yes">
            <dateCreated><%=manifest_row[:date]%></dateCreated>
          </originInfo>
        <% end %>
        <titleInfo>
          <title><% if !manifest_row[:label].blank? %><%=manifest_row[:label]%><% end %></title>
        </titleInfo>
        <identifier type="local" displayLabel="Revs ID">[[sourceid]]</identifier>
        <% if !manifest_row[:description].blank? %><note displayLabel="Description"><%=manifest_row[:description]%></note><% end %>  
        <% if !manifest_row[:model_year].blank? %><note displayLabel="Model Year" ID="model_year"><%=manifest_row[:model_year]%></note><% end %>
        <% if !manifest_row[:group_or_class].blank? %><note displayLabel="Group or Class" ID="group"><%=manifest_row[:group_or_class]%></note><% end %>
        <% if !manifest_row[:race_data].blank? %><note displayLabel="Race Data" ID="race_data"><%=manifest_row[:race_data]%></note><% end %>
        <% if !manifest_row[:metadata_sources].blank? %><note displayLabel="Metadata Sources" ID="metadata_sources"><%=manifest_row[:metadata_sources]%></note><% end %>
        <% if !manifest_row[:vehicle_markings].blank? %><note displayLabel="Vehicle Markings" ID="vehicle_markings"><%=manifest_row[:vehicle_markings]%></note><% end %>
        <% if !manifest_row[:inst_notes].blank? %><note type="source note" displayLabel="Institution Notes" ID="inst_notes"><%=manifest_row[:inst_notes]%></note><% end %>
        <% if !manifest_row[:prod_notes].blank? %><note type="source note" displayLabel="Production Notes" ID="prod_notes"><%=manifest_row[:prod_notes]%></note><% end %>
