...
2.1.1:: More updates to remediation framework
...
1. Create a TXT file containing a list of druids you wish to remediate, one line per druid.  These should be full druids, e.g.
"druid:oo000oo0001".  Save it as a .CSV file with only this single druid column (and no header) and save it somewhere the pre-assembly
code can read it.
...
run in test or development mode if you can access those environments).  Pass in the CSV file with DRUIDs and the Ruby file you generated
in steps 1 and 2.  You will get two output files -- the first is a CSV file which includes columns indicating if remediation succeeded, a message
and a timestamp.  You will also get a .log file.  Each file is named in the same way as the input file with _output.csv or _log appended to the
...
ROBOT_ENVIRONMENT=production nohup bin/remediate CSV_FILE_WITH_DRUIDS.csv REMEDIATE_LOGIC_FILENAME.rb &
...
=== Custom Object Remediation With Your Own DRUID lookup and logging

You can also build a fully customized remediation script that does not require input DRUIDs in a CSV.  To do this, you will need to implement
any logging  or resume capability you will need.  You will also still implement your own project specific Ruby file (as in step 2 above) 
but you will implement your own mechanism for generating druids to remediate and for logging their completion. 
...
file, and it will return you success or failure and a message:
