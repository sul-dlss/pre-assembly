...
In addition, the entire MODs template is passed through an ERB parser, allowing you to utilize Ruby code in the template using the standard {ERB template <tt><% %></tt>}[http://ruby-doc.org/stdlib-1.9.3/libdoc/erb/rdoc/ERB.html] syntax.  This can be used to peform more complex operations.  If you utilize Ruby code, you will have access to a special local variable called 'manifest_row', which is a hash of values for that row in the manifest, keyed off the column names.  For example:
...
To use a different character encoding in your ERB template, put the following at the top of your <tt>template.xml</tt>:

  <%#-*- coding: UTF-8 -*-%>
  <?xml version="1.0" encoding="<%= \_\_ENCODING\_\_ %>"?>
