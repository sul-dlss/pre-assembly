...
8. Running in the production environment:
   ...
   using the <tt>--resume</tt> switch, it will be overwritten. You will need the progress log for cleanup and
   restarting.
...
In addition, the entire MODs template is passed through an ERB parser, allowing you to utilize Ruby code in the template using the standard <% %> syntax.  This can be used to peform more complex operations.  If you utilize Ruby code, you will have access to a special local variable called 'manifest_row', which is a hash of values for that row in the manifest, keyed off the column names.  For example:
